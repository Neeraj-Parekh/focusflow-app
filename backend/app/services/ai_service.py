...
class AIService:
    ...
    def _load_models(self):
        ...
        try:
            self.productivity_model = joblib.load(
                f"{self.models_dir}/productivity_model.pkl"
            )
            self.priority_model = joblib.load(f"{self.models_dir}/priority_model.pkl")
            self.focus_patterns_model = joblib.load(
                f"{self.models_dir}/focus_patterns_model.pkl"
            )
            self.burnout_model = joblib.load(f"{self.models_dir}/burnout_model.pkl")
            self.productivity_scaler = joblib.load(
                f"{self.models_dir}/productivity_scaler.pkl"
            )
    ...
    def _save_models(self):
        ...
        joblib.dump(
            self.productivity_model, f"{self.models_dir}/productivity_model.pkl"
        )
        joblib.dump(self.priority_model, f"{self.models_dir}/priority_model.pkl")
        joblib.dump(
            self.focus_patterns_model, f"{self.models_dir}/focus_patterns_model.pkl"
        )
        joblib.dump(self.burnout_model, f"{self.models_dir}/burnout_model.pkl")
        joblib.dump(
            self.productivity_scaler, f"{self.models_dir}/productivity_scaler.pkl"
        )
        joblib.dump(self.priority_scaler, f"{self.models_dir}/priority_scaler.pkl")

    async def predict_optimal_work_time(
        ...
        df["productivity_score"] = df["focus_score"] * (
            df["actual_duration"] / df["planned_duration"]
        )

        # Find peak productivity hours
        hourly_productivity = (
            df.groupby("hour")["productivity_score"].mean().sort_values(ascending=False)
        )
    ...
    async def suggest_task_priority(
        self, task_data: Dict, user_context: Dict
    ) -> Dict[str, Any]:
        ...
        gpt_suggestion = await self._get_gpt_priority_suggestion(
    ...
    async def _get_gpt_priority_suggestion(
        self, task_data: Dict, user_context: Dict
    ) -> Dict[str, Any]:
    ...
    def _extract_task_features(
        ...
        if due_date:
            ...
            urgency = max(
                0, min(1, 1 / (days_to_due + 1))
            )  # Higher urgency for sooner deadlines
        else:
        ...
        features.append(
            user_context.get("active_tasks", 0) / 10
        )  # Workload (normalized)
        features.append(
            user_context.get("recent_productivity_score", 75) / 100
        )  # Recent performance
    ...
    def _rule_based_priority(self, task_data: Dict) -> tuple[str, float]:
        ...
        if any(
            keyword in project_name for keyword in ["urgent", "critical", "important"]
        ):
            score += 0.2
    ...
    async def generate_productivity_insights(
        self, user_id: str, analytics_data: Dict
    ) -> Dict[str, Any]:
    ...
    def _extract_recommendations(self, gpt_content: str) -> List[str]:
        ...
        for line in lines:
            ...
            if line and any(
                ...
                for keyword in ["recommend", "suggest", "try", "consider", "improve"]
            ):
    ...
    def _generate_fallback_insights(self, analytics_data: Dict) -> List[str]:
        ...
        if completed_pomodoros > 0:
            insights.append(
                f"You've completed {completed_pomodoros} focused work sessions!"
            )

        productivity_score = analytics_data.get("productivity_score", 0)
        if productivity_score >= 80:
            insights.append(
                "Your productivity score is excellent! Keep up the great work."
            )
        elif productivity_score >= 60:
            insights.append(
                "Good productivity levels. Consider optimizing your peak hours."
            )
        else:
            insights.append(
                "There's room for improvement. Try shorter, more focused sessions."
            )

        return insights

    async def predict_task_duration(
        self, task_description: str, user_history: List[Dict]
    ) -> int:
    ...
    def _extract_duration_features(self, description: str) -> List[float]:
        ...
        complexity_score = sum(
            1 for word in complexity_words if word in description.lower()
        )
