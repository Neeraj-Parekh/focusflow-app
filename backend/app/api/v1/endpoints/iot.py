...
async def create_automation_rule(
    ...
    device = db.query(IoTDevice).filter(
        IoTDevice.id == rule_data.device_id,
        IoTDevice.user_id == current_user.id
    ).first()
    ...
    presets = [
        {
            ...
            "actions": [
                {"device_type": "light", "action": "set_brightness", "params": {"brightness": 80}},
                {"device_type": "light", "action": "set_color", "params": {"color": "#ffffff"}},
                {"device_type": "speaker", "action": "play_ambient", "params": {"sound": "rain"}},
        ...
        },
        {
            ...
            "actions": [
                {"device_type": "light", "action": "set_brightness", "params": {"brightness": 60}},
                {"device_type": "light", "action": "set_color", "params": {"color": "#ffa500"}},
                {"device_type": "speaker", "action": "play_ambient", "params": {"sound": "nature"}},
            ]
        },
        {
            "id": "meeting_mode",
            ...
            "actions": [
                {"device_type": "light", "action": "set_brightness", "params": {"brightness": 90}},
                {"device_type": "light", "action": "set_color", "params": {"color": "#ffffff"}},
...
async def activate_environment_preset(
    ...
    # Get user's devices
    devices = db.query(IoTDevice).filter(
        IoTDevice.user_id == current_user.id,
        IoTDevice.is_online == True
    ).all()
        ...
        for device in devices:
            if preset_id == "focus_mode":
                if device.device_type == "light":
                    actions_executed.append(f"Set {device.device_name} brightness to 80%")
                elif device.device_type == "speaker":
                    actions_executed.append(f"Started ambient sound on {device.device_name}")
            # ... other presets
...
async def get_device_status(
    ...
    mock_status = {
        ...
        "properties": {
            ...
            "temperature": 22.5,
            "humidity": 45
        } if device.device_type == "light" else {
            "volume": 50,
            "playing": False,
            "current_track": None
        } if device.device_type == "speaker" else {}
    }
